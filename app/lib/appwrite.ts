...
const getProjectsCollectionId = () => document.querySelector('html')?.getAttribute('data-appwrite-projects-collection-id');
const getMessagesCollectionId = () => document.querySelector('html')?.getAttribute('data-appwrite-messages-collection-id');
...
export const getProjectByRepositoryName = async (repositoryName: string) => {
  ...
  const projects = await getDatabases().listDocuments<Project>(databaseId, projectsCollectionId, [Query.equal('repositoryName', repositoryName)]);
...
export const getMessagesByRepositoryName = async (repositoryName: string) => {
  ...
  const messages = await getDatabases().listDocuments<Message>(databaseId, messagesCollectionId, [Query.equal('project', project.$id)]);
